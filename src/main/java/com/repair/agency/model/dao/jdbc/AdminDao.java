...
import com.repair.agency.model.dao.Sql;
...
public class AdminDao {
    ...
    public boolean updateInvoiceEngineer(int engineerId, int invoiceId) {
        ...
        try (Connection con = getConnection();
             PreparedStatement prepStmt = con.prepareStatement(Sql.UPDATE_INVOICE_ENGINEER)) {
    ...
    public boolean updateInvoicePrice(String price, int invoiceId) {
        boolean result = false;
        try (Connection con = getConnection();
             PreparedStatement prepStmt = con.prepareStatement(Sql.UPDATE_INVOICE_PRICE)) {
    ...
    public boolean updateInvoiceStatus(String status, int invoiceId) {
        boolean result = false;
        try (Connection con = getConnection();
             PreparedStatement prepStmt = con.prepareStatement(Sql.UPDATE_INVOICE_STATUS)) {
    ...
    public List<Invoice> selectAllInvoices() {
        ...
        try (Connection con = getConnection();
            ...
            ResultSet rs = stmt.executeQuery(Sql.SELECT_INVOICES);
            while (rs.next()) {
    ...
    public Invoice getInvoiceById(int id) {
        ...
        try (Connection con = getConnection();
             PreparedStatement prepStmt = con.prepareStatement(Sql.FINED_INVOICE_BY_ID)) {
    ...
    public boolean setUsersWallet(String userId, String walletValue) {
        ...
        try (Connection con = getConnection();
            PreparedStatement prepStmt = con.prepareStatement(Sql.SET_USERS_WALLET)) {
...
//        System.out.println(dateFormat.format(currentDate));
    ...
    public List<Invoice> getInvoiceByStatus(String status) {
        List<Invoice> invoicesList = new ArrayList<>();
        try (Connection con = getConnection();
             PreparedStatement prepStmt = con.prepareStatement(Sql.SELECT_INVOICES_BY_STATUS)) {
            prepStmt.setString(1, status);
            ResultSet rs = prepStmt.executeQuery();
            while (rs.next()) {
                Invoice invoice = util.getInvoice(rs);
